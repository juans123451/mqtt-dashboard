...
const MQTT_TOPIC_MOV = "mi_casa/alerta/movimiento";
...
client.on("connect", () => {
  ...
  client.subscribe(
    [MQTT_TOPIC_SENSORES, MQTT_TOPIC_LED_STATUS, MQTT_TOPIC_MOV],
    (err) => {
      if (err) {
        console.error("❌ Error al suscribirse a topics:", err);
      } else {
        console.log(
          `📡 Suscrito a: ${MQTT_TOPIC_SENSORES}, ${MQTT_TOPIC_LED_STATUS}, ${MQTT_TOPIC_MOV}`
        );
      }
    }
  );
...
app.post("/api/chat", async (req, res) => {
  ...
  if (!message || !sensorData) {
    return res
      .status(400)
      .json({ error: "Faltan datos: 'message' y 'sensorData'." });
  }
...
- Temperatura ambiente: ${
    typeof sensorData.temperatura_dht === "number"
      ? sensorData.temperatura_dht.toFixed(1)
      : "--"
  } °C
- Humedad relativa: ${
    typeof sensorData.humedad_dht === "number"
      ? sensorData.humedad_dht.toFixed(1)
      : "--"
  } %
- Distancia (tanque agua): ${
    typeof sensorData.distancia_cm === "number"
      ? sensorData.distancia_cm.toFixed(1)
      : "--"
  } cm
